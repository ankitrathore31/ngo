...
@section('content')
    <div class="wrapper">
        ...
        <div class="container mt-4">
            <form method="POST" action="{{ route('update.project.report',$report->id) }}">
                @csrf
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                            ...
                            <select class="form-control @error('session') is-invalid @enderror" name="session"
                                ...
                                @foreach ($data as $session)
                                    <option value="{{ $session->session_date }}"
                                        {{ old('session', $report->academic_session) == $session->session_date ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="report" id="report" rows="5" class="form-control @error('report') is-invalid @enderror">
                                {{ old('report', $report->report) }}</textarea>
                            @error('report')
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="mission" id="mission" rows="5" class="form-control @error('mission') is-invalid @enderror">
                                {{ old('mission', $report->mission) }}</textarea>
                            @error('mission')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="conclusion" id="conclusion" rows="5"
                                ...
                                {{ old('conclusion', $report->conclusion) }}</textarea>
                            @error('conclusion')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="card mb-4">
                            ...
                            <tbody>
                                @foreach ($report->items as $i => $item)
                                    <tr>
                                        ...
                                        <td><input type="text" name="items[{{ $i }}][category]"
                    ...
                    <div class="card-footer text-end">
                        <strong>Total Amount: ₹<span
                                id="total-amount">{{ number_format($report->items->sum('expense'), 2) }}</span></strong>
                    </div>
                </div>

                <div class="text-end">
                    <button type="submit" class="btn btn-success">Update Project Report</button>
                </div>
            </form>
        </div>
